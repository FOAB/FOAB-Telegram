...
/** Feature settings exposed by both the Mini App and inline-button fallback. */
export type SettingsFeature = 'welcome' | 'rules' | 'goodbye';

/** Delivery modes shared by the Mini App and inline settings fallback. */
...
export type SettingsCallback =
  ...
  | { readonly kind: 'enable-feature'; readonly feature: SettingsFeature }
  | { readonly kind: 'edit-feature'; readonly feature: SettingsFeature }
  | { readonly kind: 'disable-feature'; readonly feature: SettingsFeature }
  | { readonly kind: 'set-feature-mode'; readonly feature: SettingsFeature; readonly mode: MessageDeliveryMode }
  | { readonly kind: 'toggle-feature-delete'; readonly feature: SettingsFeature }
  | { readonly kind: 'set-locale'; readonly value: SupportedLocale }
  | { readonly kind: 'set-time-zone'; readonly value: string }
  | { readonly kind: 'back' }
  | { readonly kind: 'close' };
...
export function settingsCallbackData(action: SettingsCallback): string {
  switch (action.kind) {
    ...
    case 'show-feature':
      return `foab:settings:feature:${action.feature}`;
    case 'enable-feature':
      return `foab:settings:enable-feature:${action.feature}`;
    case 'edit-feature':
      return `foab:settings:edit-feature:${action.feature}`;
    case 'disable-feature':
      return `foab:settings:disable-feature:${action.feature}`;
    case 'set-feature-mode':
      return `foab:settings:feature-mode:${action.feature}:${action.mode}`;
    case 'toggle-feature-delete':
      return `foab:settings:toggle-feature-delete:${action.feature}`;
    case 'set-locale':
...
export function parseSettingsCallbackData(input: string): SettingsCallback | null {
  ...
  if (prefix !== 'foab' || settings !== 'settings' || (rest.length > 0 && action !== 'feature-mode') || !value) {
    return null;
  }
  ...
  if (action === 'feature') {
    return isSettingsFeature(value) ? { kind: 'show-feature', feature: value } : null;
  }
  if (action === 'edit-feature') {
    return isSettingsFeature(value) ? { kind: 'edit-feature', feature: value } : null;
  }
  if (action === 'enable-feature') {
    return isSettingsFeature(value) ? { kind: 'enable-feature', feature: value } : null;
  }
  if (action === 'disable-feature') {
    return isSettingsFeature(value) ? { kind: 'disable-feature', feature: value } : null;
  }
  if (action === 'toggle-feature-delete') {
    return isSettingsFeature(value) ? { kind: 'toggle-feature-delete', feature: value } : null;
  }
  if (action === 'feature-mode') {
    const [feature, mode, ...extra] = [value, ...rest];
    return extra.length === 0 && mode !== undefined && isSettingsFeature(feature) && isMessageDeliveryMode(mode)
      ? { kind: 'set-feature-mode', feature, mode }
      : null;
  }
  if (action === 'locale') {
...
/** Accepts only the three feature keys generated by FOAB's settings keyboards. */
function isSettingsFeature(value: string): value is SettingsFeature {
  return value === 'welcome' || value === 'rules' || value === 'goodbye';
}

/** Accepts only the persisted delivery modes. */
