...
At this checkpoint, **T-00, T-01, T-02, T-03, and the repository security foundation are implemented; T-02 and T-03 are verified against local PostgreSQL; F-001 has a locally tested implementation slice; and no full moderation or protection capability is implemented.** Update this statement and individual statuses as evidence changes. Do not mark a product capability verified until its behavior, authorization boundaries, multi-group isolation, failure modes, localization, and required external checks have evidence.
