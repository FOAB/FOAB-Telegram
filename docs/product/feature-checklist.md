...
- [ ] **F-001 · P0 — Onboarding, help, and settings.** `/start`, `/help`, `/settings`, and `/cancel`; contextual help; private start guidance; cancellation affects only the active flow. **Status: In progress** — all four command handlers, scoped group settings with optimistic revisions, current exact-group administrator checks, localized menus, and transient-flow cancellation exist; live Telegram verification, private group selection, and broader contextual help remain.
...
At this checkpoint, **T-00, T-01, T-02, and the repository security foundation are implemented; T-02 is verified against local PostgreSQL; F-001 has a locally tested implementation slice; and no full moderation or protection capability is implemented.** Update this statement and individual statuses as evidence changes. Do not mark a product capability verified until its behavior, authorization boundaries, multi-group isolation, failure modes, localization, and required external checks have evidence.
