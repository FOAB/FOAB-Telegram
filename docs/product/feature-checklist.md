...
- [ ] **F-060 · P1 — Administration Mini App.** Use the same API and authorization as commands; provide drafts, diff, save, and conflict handling. **Status: In progress** — HTTPS URL gating, private-only launch placement, and signed Telegram `initData` verification exist; the schema-validated API, session lifecycle, UI, and write routes remain. Inline callback keyboards are the fallback.
