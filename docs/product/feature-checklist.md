...
- [ ] **F-001 · P0 — Onboarding, help, and settings.** `/start`, `/help`, `/settings`, `/reload`, and `/cancel`; contextual help; private start guidance; cancellation affects only the active flow. **Status: In progress** — onboarding, help, read-only diagnostics, settings, current-group reload, cancellation, scoped group settings with optimistic revisions, current exact-group administrator checks, localized menus, private language selection, transient-flow cancellation, and private group selection exist; live Telegram verification and broader contextual help remain.
...
- [ ] **F-006 · P1 — Welcome messages.** Shared templates for group/private delivery, topics, previous-message cleanup, raid batches, and post-challenge admission; deduplicate overlapping join updates and handle unavailable DMs. **Status: In progress** — the Mini App and inline fallback both support activation without erasing saved text, bounded text editing, always/first-entry delivery mode, and optional deletion of FOAB's previous welcome message; observed new-member delivery is implemented. Templates, media/URL buttons, topics, raid/admission integration, deduplication, and live delivery verification remain.
- [ ] **F-007 · P1 — Goodbye messages.** Configurable template, destination, TTL, and distinct voluntary-leave/removal behavior; do not recurse when FOAB itself removes a member. **Status: In progress** — the Mini App and inline fallback both support activation without erasing saved text, bounded text editing, always/first-entry delivery mode, and optional deletion of FOAB's previous goodbye message; observed member-leave delivery is implemented. Templates, media/URL buttons, topics, TTL, removal distinctions, and live delivery verification remain.
...
- [ ] **F-060 · P1 — Administration Mini App.** Use the same API and authorization as commands; provide drafts, diff, save, and conflict handling. **Status: In progress** — HTTPS URL gating, private Menu Button and settings-flow launch placement, signed Telegram `initData` verification, a schema-validated API, short-lived server-side sessions, origin/CSRF checks, administrator-filtered searchable group listing, per-user private language settings, BotFather-style grouped settings navigation, optimistic settings writes for language, time zone, welcome, goodbye, and rules, and allowlisted responses exist. Welcome and goodbye controls are mirrored by inline callback keyboards, including activation, message editing, delivery mode, and previous-message cleanup. Durable multi-instance sessions, rate limits, richer drafts/diffs, media/template editing, browser evidence, and deployment remain.
