...
| Mini App authentication and sessions | Invalid, replayed, and expired init data; origin/CSRF; session renewal; output redaction; direct-object access denial. | Signed `initData` verification, short-lived in-memory sessions, origin/CSRF checks, allowlisted output, expiry/revocation, and cross-group denial have synthetic coverage. Durable sessions, rate limits, and browser/runtime checks remain. |
...
| Runtime security and recovery | Rate limits, retention, backup/restore, clean install, operational diagnostics, and real-client capability checks. | The Mini App API applies a body limit, restrictive response headers, no-store responses, and generic error output. Rate limits, retention, backup/restore, and real-client checks remain unimplemented or untested. |
