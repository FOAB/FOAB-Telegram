...
| Mini App authentication and sessions | Invalid, replayed, and expired init data; origin/CSRF; session renewal; output redaction; direct-object access denial. | Signed `initData` verification has synthetic coverage; HTTP sessions, origin/CSRF controls, output redaction, and direct-object access checks are not implemented or tested. |
