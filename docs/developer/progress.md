...
- **T-01 — TypeScript runtime bootstrap:** implemented with typed token/database configuration, grammY long polling, scoped onboarding/settings handlers, read-only `/ping` and `/id` diagnostics, Web App URL gating, private Menu Button selection, ephemeral group command-menu publication and requester-targeted replies, grouped inline settings fallback for language, time zone, welcome, goodbye, and rules, normal private-chat replies, safe structured logs, and database-pool cleanup. Live Telegram behavior has not been exercised in this implementation slice.
- **T-02 — PostgreSQL/Drizzle identity and group storage:** implemented and locally verified on PostgreSQL 18. The reviewed migrations create installation identity, singleton runtime state, installation-scoped group records, and installation/user-scoped private preferences. Migration, runtime, and test roles are separate; runtime roles cannot create or alter schema objects.
...
- **F-001 — Onboarding and help:** in progress. Private/group `/start`, `/help`, `/ping`, `/id`, `/settings`, `/reload`, and `/cancel`, group registration, requester-only group feedback, localized command descriptions, private language selection, current exact-group administrator checks, optimistic group settings writes, and private group selection exist. Live Telegram checks and broader help flows remain.
- **F-004 — Group rules:** in progress. Administrators can publish bounded group rules through the Mini App and members can read them with ephemeral `/rules`; acceptance flows and history remain.
- **F-006 — Welcome messages:** in progress. The Mini App and inline fallback both configure activation, bounded text, always/first-entry delivery, and deletion of FOAB's previous welcome message. FOAB sends configured text for observed new-member events and records the last automated message for the cleanup policy. Media/templates, placeholders, topics, deduplication, and live Telegram delivery remain.
- **F-007 — Goodbye messages:** in progress. The Mini App and inline fallback both configure activation, bounded text, always/first-entry delivery, and deletion of FOAB's previous goodbye message. FOAB sends configured text for observed member-leave events and records the last automated message for the cleanup policy. Media/templates, placeholders, topic/TTL behavior, and live Telegram delivery remain.
...
- **F-060 — Administration Mini App:** in progress. The optional HTTPS URL boundary, private Menu Button and settings-flow launch, signed `initData` verifier, schema-validated HTTP API, short-lived server-side session lifecycle, origin/CSRF controls, allowlisted group responses, administrator-filtered searchable group listing, private per-user language settings, BotFather-style grouped navigation, optimistic settings writes, and a React/Vite editor with direct welcome, goodbye, and rules editors exist for language, time zone, welcome, goodbye, and rules configuration. Durable multi-instance sessions, rate limits, broader administration screens, browser/runtime evidence, and deployment remain. Inline callback keyboards are the fallback.
- **Deployment packaging:** implemented with a multi-stage Node 24 Dockerfile, a Dokploy-compatible Compose definition, a liveness endpoint, and an English deployment runbook. Docker image builds and remote Dokploy routing remain external verification gates.
- **F-065 — Ephemeral command feedback:** in progress. The group command registry opts `/start`, `/help`, `/ping`, `/id`, `/settings`, `/reload`, and `/cancel` into ephemeral delivery; response payloads target the authenticated sender and reply to the incoming ephemeral message. Private-chat responses use ordinary messages. Telegram eligibility and client delivery remain unverified; failed private delivery never falls back to a public group response.
...
- `pnpm db:generate` — passed; generated the reviewed welcome/goodbye delivery migration and the installation/user-scoped private preferences migration.
...
- `pnpm check` — passed; strict TypeScript checks, 73 unit tests, and the webapp typecheck cover signed Web App sessions, private locale persistence, origin/CSRF handling, cross-group denial, settings callbacks, and Mini App/fallback parity.
- `pnpm build` — passed after the private bot settings API, database repository, and Mini App/fallback changes.
- The settings slice added `pnpm db:generate`, `pnpm db:migrate`, and `pnpm db:migrate:test` coverage for group settings revisions and private user preferences; integration tests cover stale and cross-installation settings writes.
...
- Group records are keyed and queried by server-owned installation UUID plus Telegram chat ID. The database stores group metadata, bot membership state, versioned group language/time-zone settings, and the selected private locale for each installation/user pair; message bodies and member names are not persisted.
...
2. Finish F-001/F-002 with live Telegram validation, bounded pagination, private administrator reload, and contextual help.
